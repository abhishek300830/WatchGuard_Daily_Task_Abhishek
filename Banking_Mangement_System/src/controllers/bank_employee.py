    ...
    def check_for_withdraw_requests(self):
        ...
        if all_requests:
            for request in all_requests:
                ...
                self.__approve_or_reject_withdrawn_requests(user_id, request_id, done_by, amount)
        else:
            print("No pending Withdrawn Requests.")

    def __approve_or_reject_withdrawn_requests(self, user_id, request_id, done_by, amount):
        while True:
    ...
    def check_for_new_registration_request(self):
        ...
        if not_verified_user_details:
            for user in not_verified_user_details:
                ...
                self.__approve_or_reject_registeration_request(user_id)


        else:
            print("No Pending Registration Requests.")

    def __approve_or_reject_registeration_request(self, user_id):
        while True:
            manager_choice = input("1. Approve\n2. Reject \n Enter (1,2) : ")
            if manager_choice == '1':
                update_item(queries.UPDATE_VERIFIED_STATUS_OF_CUSTOMER, ("1", user_id))
                break
            if manager_choice == '2':
                update_item(queries.UPDATE_VERIFIED_STATUS_OF_CUSTOMER, ("2", user_id))
                break
            else:
                print("Please Enter Valid Input...")
    ...
    def check_for_user_modification_request(self):
        ...
        if pending_modification_requests:
            for request in pending_modification_requests:
                ...
                self.__approve_or_reject_modification_requests(attribute_to_update, attribute_value, user_id,
                                                               request_id)

        else:
            print("No Pending User Modification Requests.")

    def __approve_or_reject_modification_requests(self, attribute_to_update, attribute_value, user_id, request_id):
        while True:
            manager_choice = input("1. Approve\n2. Reject \n Enter (1,2) : ")
            if manager_choice == '1':
